...
- Noul `has_security_risk`・`has_maintainability_risk` ── 確率 50% 以上のとき「セキュリティリスク 97%」のように**シグナル**として表示
...
次に入力検証の `if` を消しました。予想は「もっと悪くなる」。結果は REJECT 46、判定は CAUTION。**下がった**んです。保守性リスクは 74% のまま、セキュリティリスクは表示されず（50% 未満）。
...
| --- | ---: | ---: | ---: |
| Clean（参考・1 回） | 42 | 50% 未満（非表示） | 57% |
| `any` | 48.0 | 50% 未満（非表示） | 72〜74% |
| 入力検証を削除 | 35.7 | 50% 未満（非表示） | 53〜56% |
| 例外を握りつぶす | 67.3 | 50% 未満（非表示） | 74〜75% |
| SQL 文字列結合 | 100 | 97% | 92〜93% |
| `eval()` | 100 | 93〜94% | 92〜93% |

※ アプリは Noul の確率が 50% 以上のときだけシグナルとして表示し、50% 未満の値は保存していません（`lib/jev.ts`）。「50% 未満」は Jev が答えなかったという意味ではなく、Yes の確率が半分に届かなかったという意味です。
