---
title: 判断特化AI「Jev」の境界を 48 回壊して測った ― 0.4 秒・ほぼ 0 円の本番可否判定を CI に置く前に
tags:
  - Jev
  - TypeScript
  - Next.js
...
「このコード、本番に出していい？」と聞くと 0.4 秒後に校正済みの確率が返ってきて、48 回聞いても $0.0012（Gateway のプロモーション期間中なら 0）。そんな審査員が手元にいれば、コードレビューのやり方が変わると思っています。人が全部読むのをやめて、機械が迷ったところ、つまり境界の上に乗ったコードだけを人が読む。安いので 1 回では決めず、30 回聞いて分布で決める。

ただ、その審査員が何に反応して、どこで判定を変えて、どれくらいブレるのかを知らないまま CI に入れると、境界の上のコードを機械的に落とすだけの装置になります。だからこの記事では、使い方の前に、まず審査員の「境界」と「癖」を測りました。
...
冒頭の「機械が迷ったところだけ人が読む」レビューに近づけるために、観察した事実からそのまま導ける範囲だけ書きます。
...
冒頭のゴールに照らすと、今回できたのは「境界を測る」ところまでです。何に反応するか（セキュリティのシグナルが点いたのは SQL 結合と `eval()` だけで、残りは保守性のシグナルが動く）、どこで判定を変えるか（SQL 結合で 100）、どれくらいブレるか（境界の上で 1 点差）は数字で押さえられました。そこから見えた「信用できる使い方」もはっきりしています。説明を返さないぶん 1 回 360 ms（中央値）・出力 85 トークンで、48 回投げても $0.0012（Gateway のプロモーション期間中は 0）。この速さと安さは「1 回聞いて信じる」ためではなく、**何度も聞いて分布を見る**ため、差分単位で聞いて天井を避けるため、シグナルで人が見る順番を決めるために使うものだと思います。

足りないものは 2 つ。生の確率と `confidence` を保存していないので境界の 1 件を追えないこと、サンプルが 1 種類の関数だけなこと。次は生の確率を保存して境界の 1 件を追いかけ、質問文を変えて「入力検証を消すと下がる」が消えるかを確かめ、自分のリポジトリの正常なコードでベースラインを取るところからやります。
